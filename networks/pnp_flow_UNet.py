# from https://github.com/annegnx/PnP-Flow/blob/main/pnpflow/models.py
...
def group_norm(out_ch):
    ## SHAHRIAR EDIT
    num_groups = 32
    # If out_ch is smaller than 32 or not divisible by 32, adjust num_groups downwards.
    if out_ch < num_groups:
        num_groups = out_ch  # each channel in its own group, if needed
    else:
        # Decrease num_groups until it divides out_ch
...
class UNet2(nn.Module):
    def __init__(self,
        ...
        # Shahriar edit: manually scale the weights of the last layer
        with torch.no_grad():
            self.end_conv[-1].weight.mul_(1.0) #0.5 
            if self.end_conv[-1].bias is not None:
                self.end_conv[-1].bias.mul_(1.0) #0.5
    # noinspection PyMethodMayBeStatic
    def _compute_cond_module(self, module, x, temp):
        for m in module:
            x = m(x, temp)
