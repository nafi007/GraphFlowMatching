...
def DiT_S_8(**kwargs):
    return DiT(depth=12, hidden_size=384, patch_size=8, num_heads=4, **kwargs)

# SHAHRIAR EDIT 

# def DiT_XS_2(**kwargs):
#     return DiT(depth=12, hidden_size=300, patch_size=2, num_heads=10, **kwargs)

def DiT_XS_2(**kwargs):
    return DiT(depth=13, hidden_size=300, patch_size=2, num_heads=10, **kwargs)

def DiT_XS_2_noDiffusion(**kwargs):
    # for use with no Diffusion case. Same as DiT_XS_2, but +1 additional depth
    return DiT(depth=13, hidden_size=300, patch_size=2, num_heads=10, **kwargs)

def DiT_M_2_noDiffusion(**kwargs):
    """
    DiT_M/2: A medium-sized DiT model with approximately 150 million parameters.
    Configuration: depth=22, hidden_size=768, patch_size=2, num_heads=12.
    """
    return DiT(depth=14, hidden_size=768, patch_size=2, num_heads=12, **kwargs)
...
DiT_models = {
    ...
    "DiT-S/4": DiT_S_4,
    "DiT-S/8": DiT_S_8,
    "DiT-XS/2": DiT_XS_2, #SHAHRIAR EDIT
    "DiT-XS/2_noDiffusion": DiT_XS_2_noDiffusion, #SHAHRIAR EDIT
    "DiT-M/2_noDiffusion": DiT_M_2_noDiffusion
}