...
from networks.DiT import LabelEmbedder
    ...
    def forward(self, x, emb, **kwargs):
        ...
        if self.num_heads:
            ...
            if not self.use_mem_efficient_attn or not _HAS_FUSED_ATTN:
                ...
                a = torch.nn.functional.scaled_dot_product_attention(q, k, v, dropout_p=0.0)
    ...
    def __init__(
        ...
        if self.use_transformer:
            self.transformer = TransformerBlock(
                ...
                init_zero=init_zero,
                init_attn=init_attn,
                use_mem_efficient_attn=use_mem_efficient_attn,
            )
...
class FeedForward(torch.nn.Module):
    def __init__(self, in_channels, out_channels=None, channel_mult=4, init=dict()):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        return x.permute(0, 2, 1).view(x.size(0), -1, int(x.size(1) ** 0.5), int(x.size(1) ** 0.5))
...
class TransformerBlock(torch.nn.Module):
    def __init__(
        ...
        self.attn1 = CrossAttention(
            in_channels, None, num_heads, channels_per_head, eps, init, init_zero, init_attn, use_mem_efficient_attn
        )
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f"{res}x{res}_down"] = resblock(
                    in_channels=cout, out_channels=cout, down=True, **block_kwargs
                )
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                self.enc[f"{res}x{res}_block{idx}"] = resblock(
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = img_resolution >> level
            if level == len(channel_mult) - 1:
                self.dec[f"{res}x{res}_in0"] = resblock(
                    in_channels=cout, out_channels=cout, attention=True, **block_kwargs
                )
            ...
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                self.dec[f"{res}x{res}_block{idx}"] = resblock(
                    in_channels=cin, out_channels=cout, attention=(res in attn_resolutions), **block_kwargs
                )
